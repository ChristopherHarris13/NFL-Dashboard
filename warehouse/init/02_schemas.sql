-- Medallion layout. Runs against the `warehouse` database on first boot.
--
...
--   meta    pipeline bookkeeping (ingest cursors, run audit)
...
COMMENT ON SCHEMA bronze IS 'Raw vendor payloads as JSONB, append-only. Never cleaned in place.';
...
COMMENT ON SCHEMA meta   IS 'Pipeline state: vendor cursors, ingest audit.';
